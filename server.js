...
function reconstructFormDataFromInfoText(shareInfoText) {
  ...
  const LABEL_TO_FIELD = {
    ...
    // LAP Business additional labels
...
// POST /api/new-lead-alert  — external new-lead alert webhook (cross-CRM mirror)
// Lets a peer CRM (e.g. Swiggy) tell THIS CRM's admin panel to play the exact
// same "new lead" alert sound it plays on its own dashboard. We simply relay a
// 'new-lead' event to the admin-room; the admin panel handles it identically
// to a local new lead. This is entirely SEPARATE from the voice-announcement
// (TTS) system and does not touch it. Optionally guarded by
// ANNOUNCE_WEBHOOK_TOKEN (if set, callers must send a matching { token } or
// X-Announce-Token header — otherwise the endpoint is open for LAN/peer use).
app.post('/api/new-lead-alert', (req, res) => {
  ...
  const body = req.body || {};
  const count = Number(body.count) || (Array.isArray(body.leads) ? body.leads.length : 0) || 1;
  const leads = Array.isArray(body.leads) ? body.leads.slice(0, 20) : [];
  const source = body.source ? String(body.source).slice(0, 60) : 'peer-crm';
  io.to('admin-room').emit('new-lead', { count, leads, source });
  console.log(`🔔 External new-lead alert from ${source}: ${count} lead(s)`);
  res.json({ ok: true });
});
